...
class Util {
	/**
	 * Returns html markup for a custom WP Post query
	 *
	 * @param array $args
	 */
	static function custom_query( $args = array() ) {
	 ...
	 * @param bool   $link
	 */
	static function the_title( $class = 'is-3', $link = true ) {
		$heading      = static::_convert_heading( $class );
	...
	/**
	 * Returns the HTML heading type based on class
	 *
	 * @param $class
	 *
	 * @return string
	 */
	static function _convert_heading( $class ) {
		switch ( $class ) {
			case 'is-1':
				$heading = 'h1';
				break;
	...
	/**
	 * Prints the comments template if params are met
	 */
	static function the_comments() {
